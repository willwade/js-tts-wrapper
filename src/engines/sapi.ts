...
export class SAPITTSClient extends AbstractTTSClient {
  ...
  async synthToBytes(text: string, options?: SpeakOptions): Promise<Uint8Array> {
    try {
      ...
      // Ensure we have valid SSML without creating nested <speak> tags
      if (!processedText.trim().startsWith("<speak")) {
      ...
      // Always use SSML processing since we now enforce SSML format
  ...
  async synthToBytestream(
    ...
    try {
      ...
      processedText = this.ensureProperSSML(processedText);
      const plainText = this.stripSSML(processedText);
