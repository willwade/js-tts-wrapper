...
export class LanguageNormalizer {
  ...
  static normalize(langCode: string, countryCode?: string): StandardizedLanguage {
    try {
      ...
      const iso639_3 = this.iso1To3[language] || language;
      
      // Create BCP-47 tag
      const bcp47 = region ? `${language}-${region}` : language;
      
      // Create display name
      let display = this.languageNames[language] || language;
      if (region && this.regionNames[region]) {
        display += ` (${this.regionNames[region]})`;
      } else if (region) {
        display += ` (${region})`;
      }
  ...
  static getDisplayName(langCode: string): string {
    return this.normalize(langCode).display;
  }
  ...
  static getISO639_3(langCode: string): string {
    return this.normalize(langCode).iso639_3;
  }
  ...
  static getBCP47(langCode: string, countryCode?: string): string {
    return this.normalize(langCode, countryCode).bcp47;
  }
}
